...
class GenomicTokenizer(BaseTokenizer):
    def __init__(self, lang='en', ngram=5, stride=2, frame_mod:Learner=None):
        self.lang = lang
        self.ngram = ngram
        self.stride = stride
    ...
    def tokenizer(self, t):
        ...
        if self.ngram == 1:
            ...
            if self.frame_mod is not None:
                #predict frame 
                frame = int(str(self.frame_mod.predict(t)[0]))
                if frame < 0: #if neg frame, take reverse complement 
                    t = str(Seq(t).reverse_complement())
                frame_start = np.abs(frame)-1
                toks = [t[i:i+self.ngram] for i in range(frame_start, len(t), self.stride) if len(t[i:i+self.ngram]) == self.ngram]
            else:
                #if no frame_mod, just start at position 1
