...
def get_response(query):
    ...
    fout = '/scratch/ah1114/LoL/TransferLearningTasks/Homologs_Emb/uniprot2cds_phylum/uniprot2cds_phylum.tsv'
    params = { 
...
#*sulfolobus=Crenarchaeota=28889  --> Acidobacteria=57723
#streptomyces=Actinobacteria=201174
#pseudomonas=Proteobacteria=1224
#thermococcus=Euryarchaeota=28890
#bacillus=Firmicutes=1239
#**Methanosarcina=Euryarchaeota   --> Thaumarchaeota=651137
#**acinetobacter=Proteobacteria   --> Cyanobacteria=1117
#**Mycobacterium=Actinobacteria   --> Nitrospirae=40117  --> Chloroflexi=200795
#**Lactobacillus=Firmicutes   --> Bacteroidetes=976
#**Enterococcus=Firmicutes   --> Verrucomicrobia=74201

#Crenarchaeota (28889) and Nitrospirae (40117) not included in OG groups; so replace with somewhat closely related 

OGnames = ['Acidobacteria','Actinobacteria','Proteobacteria','Euryarchaeota','Firmicutes','Thaumarchaeota','Cyanobacteria','Chloroflexi','Bacteroidetes','Verrucomicrobia']
...
OGids = [57723,201174,1224,28890,1239,651137,1117,200795,976,74201]
'''
#Acidobacteria, Chloroflexi, Verrucomicrobia all show up as \\N... change that here
subset = pd.DataFrame()
for ix,o in enumerate(OGids):
    sub = joinedtax[joinedtax['NCBI_taxID']==o] 
    if sub['scientific_name'].iloc[0]=='\\N':
        print('replacing \\N with',OGnames[ix])
        sub['scientific_name'] = sub['scientific_name'].replace('\\N',OGnames[ix])
    subset = subset.append(sub)
...
rename.to_csv('/scratch/ah1114/LoL/TransferLearningTasks/Homologs_Emb/uniprot2cds_phylum/uniprot2cds_phylum.csv',index=False)

subsetcds = subset.merge(rename,on='externalID') 
subsetcds.to_csv('/scratch/ah1114/LoL/TransferLearningTasks/Homologs_Emb/OG2genes2cds_OGtaxa_phylum.csv',index=False)
'''

subsetcds = pd.read_csv('/scratch/ah1114/LoL/TransferLearningTasks/Homologs_Emb/OG2genes2cds_OGtaxa_phylum.csv')

print('getting and writing fasta...')
for taxa in OGnames:
    print('processing',taxa,'...')
    #get list files figure out how many OGs already retrieved
    og_path = '/scratch/ah1114/LoL/TransferLearningTasks/Homologs_Emb/OG_seqs_phylum/'+str(taxa)+'/'
