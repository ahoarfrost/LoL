...
def get_response(query):
    ...
    fout = '/scratch/ah1114/LoL/TransferLearningTasks/Homologs_Emb/uniprot2cds_family/uniprot2cds_family.tsv'
    params = { 
...
def sample_OGs(df, num_ogs=1000, seqs_per_og=5, success_ogs=[], error_ogs=[]):
    #find first <num_ogs> OGs with >= <seqs_per_og> unique orthoDB genes
    counts = Counter(df['OGID']).most_common()
    ogs = [x[0] for x in counts if x[0] not in success_ogs and x[0] not in error_ogs] #remove ogs already sampled or already failed to download
    gids = {}
    total_sampled = 0
    too_short = 0
    lengths = []
    for og in ogs:
        if total_sampled < num_ogs:
            ...
            if len(unique_genes) >= seqs_per_og:
                ...
                #for some families there are not enough OGIDs with at least 5 seqs_per_og; if this is the case, relax the seqs_per_og requirement and just take all of them
                genes = pd.Series(unique_genes)
                too_short += 1
                lengths.append(len(unique_genes))
            seqs = []
            for gene in genes:
            ...
            total_sampled += 1
    if  too_short>0:
        print('number of OGs with fewer than seqs_per_og CDS IDs:',too_short)
        print('number of CDS IDs for those OGs:',Counter(lengths))
    return gids

def get_and_write(gids, og_path, success_ogs, error_ogs):
    for og in gids.keys():
...
#**sulfolobus=Sulfolobaceae=118883   --> Desulfurococcaceae=2272
#streptomyces=Streptomycetaceae=2062
#**pseudomonas=Pseudomonadaceae=135621      --> Halobacteriaceae=2236
#thermococcus=Thermococcaceae=2259
#bacillus=Bacillaceae=186817
#Methanosarcina=Methanosarcinaceae=2206
#acinetobacter=Moraxellaceae=468
#Mycobacterium=Mycobacteriaceae=1762
#Lactobacillus=Lactobacillaceae=33958
#Enterococcus=Enterococcaceae=81852

#Sulfolobaceae (118883) and methanosarcinales (94695) not included in OG groups; so replace with somewhat closely related 

OGnames = ['Desulfurococcaceae','Streptomycetaceae','Halobacteriaceae','Thermococcaceae','Bacillaceae','Methanosarcinaceae','Moraxellaceae','Mycobacteriaceae','Lactobacillaceae','Enterococcaceae']
...
OGids = [2272,2062,2236,2259,186817,2206,468,1762,33958,81852]
...
rename.to_csv('/scratch/ah1114/LoL/TransferLearningTasks/Homologs_Emb/uniprot2cds_family/uniprot2cds_family.csv',index=False)

subsetcds = subset.merge(rename,on='externalID') 
subsetcds.to_csv('/scratch/ah1114/LoL/TransferLearningTasks/Homologs_Emb/OG2genes2cds_OGtaxa_family.csv',index=False)
'''

subsetcds = pd.read_csv('/scratch/ah1114/LoL/TransferLearningTasks/Homologs_Emb/OG2genes2cds_OGtaxa_family.csv')

print('getting and writing fasta...')
for taxa in OGnames:
    print('processing',taxa,'...')
    #get list files figure out how many OGs already retrieved
    og_path = '/scratch/ah1114/LoL/TransferLearningTasks/Homologs_Emb/OG_seqs_family/'+str(taxa)+'/'
    ...
    while len(success_ogs) < 1000:
        ...
        gids = sample_OGs(taxasubset,num_ogs=num_ogs,seqs_per_og=5,success_ogs=success_ogs, error_ogs=error_ogs)
        ...
        success_ogs,error_ogs = get_and_write(gids,og_path=og_path,success_ogs=success_ogs, error_ogs=error_ogs) 
