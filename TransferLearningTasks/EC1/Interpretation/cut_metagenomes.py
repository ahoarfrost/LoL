...
fout = '/scratch/ah1114/LoL/TransferLearningTasks/EC1/metagenomes/'+str(metagenome_id)+'_cut.fastq'
ix=0
records = []
with open(fout, 'a') as outfile:
    for record in SeqIO.parse('/scratch/ah1114/LoL/TransferLearningTasks/EC1/metagenomes/'+str(metagenome_id)+'.fastq','fastq'):
        ix += 1
        if ix <= max_seqs:
            if ix % 1000000 == 0:
