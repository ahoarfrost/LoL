...
learn.freeze()
rnd=str(0)
train_round(learn, rnd=rnd, num_cycles=4, lrate=8e-3)
...
callbacks = [SaveModelCallback(learn, every='improvement', monitor='accuracy', name=Path(model_path_base+rnd+'_best')),
                SaveModelCallback(learn, every='epoch', monitor='accuracy', name=Path(model_path_base+rnd)),
                CSVLogger(learn, filename=log_path, append=True)]
learn.fit_one_cycle(5, slice(2e-3/50,2e-3), moms=(0.8,0.7), callbacks=callbacks) #this 2.6 rule of thumb is for NLP, maybe doesn't hold... but we'll go with it for now
...
callbacks = [SaveModelCallback(learn, every='improvement', monitor='accuracy', name=Path(model_path_base+rnd+'_best')),
                SaveModelCallback(learn, every='epoch', monitor='accuracy', name=Path(model_path_base+rnd)),
                CSVLogger(learn, filename=log_path, append=True)]
learn.fit_one_cycle(8, slice(5e-4/50,5e-4), moms=(0.8,0.7), callbacks=callbacks) 
learn.unfreeze()
rnd=str(3)
callbacks = [SaveModelCallback(learn, every='improvement', monitor='accuracy', name=Path(model_path_base+rnd+'_best')),
                SaveModelCallback(learn, every='epoch', monitor='accuracy', name=Path(model_path_base+rnd)),
                CSVLogger(learn, filename=log_path, append=True)]
learn.fit_one_cycle(10, slice(5e-4/(2.6**4),5e-4), moms=(0.8,0.7), callbacks=callbacks) 
