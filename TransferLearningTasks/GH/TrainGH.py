...
path = Path('./') 
data_path = Path('/scratch/ah1114/LoL/TransferLearningTasks/GH/')
train_path = Path('/scratch/ah1114/LoL/TransferLearningTasks/GH/train/gh_train.csv')
valid_path = Path('/scratch/ah1114/LoL/TransferLearningTasks/GH/valid/gh_valid.csv')
model_path_base = '/home/ah1114/LanguageOfLife/TransferLearningTasks/GH/models/gh_clas_round'
lr_plot_base = '/home/ah1114/LanguageOfLife/TransferLearningTasks/GH/plots/lrplot_gh_clas'
losses_plot_base = '/home/ah1114/LanguageOfLife/TransferLearningTasks/GH/plots/lossesplot_gh_clas'
log_path = Path('/home/ah1114/LanguageOfLife/TransferLearningTasks/GH/train_logs/gh_clas_log')
...
cache_dir = 'tmp_gh' #otherwise your tmp recorder stuff will all go in the same place if you're running multiple jobs
...
data = BioClasDataBunch.from_df(path=data_path,train_df=train_df,valid_df=valid_df,
                tokenizer=tok, vocab=model_voc, classes=['gh','nongh'],
                text_cols='seq',label_cols='gh',
                bs=bs,device=device
                )
...
lr_plot.savefig(Path(lr_plot_base+'.png'))
...
learn.freeze()
learn.fit_one_cycle(30,1e-2, moms=moms)

learn.freeze_to(-2)
learn.fit_one_cycle(30, slice(5e-3/50,5e-3), moms=moms) 

learn.freeze_to(-3)
learn.fit_one_cycle(30, slice(5e-4/50,5e-4), moms=moms) 
...
learn.fit_one_cycle(30,1e-2, moms=(0.8,0.7), callbacks=callbacks)
...
callbacks = [SaveModelCallback(learn, every='improvement', monitor='accuracy', name=Path(model_path_base+rnd+'_best')),
                SaveModelCallback(learn, every='epoch', monitor='accuracy', name=Path(model_path_base+rnd)),
                CSVLogger(learn, filename=log_path, append=True)]
learn.fit_one_cycle(30, slice(5e-3/50,5e-3), moms=(0.8,0.7), callbacks=callbacks) # 2.6**4 (not quite 50) is a rule of thumb is for NLP, maybe doesn't hold... but we'll go with it for now
...
callbacks = [SaveModelCallback(learn, every='improvement', monitor='accuracy', name=Path(model_path_base+rnd+'_best')),
                SaveModelCallback(learn, every='epoch', monitor='accuracy', name=Path(model_path_base+rnd)),
                CSVLogger(learn, filename=log_path, append=True)]
learn.fit_one_cycle(30, slice(1e-3/50,1e-3), moms=(0.8,0.7), callbacks=callbacks) 
learn.unfreeze()
rnd=str(3)
callbacks = [SaveModelCallback(learn, every='improvement', monitor='accuracy', name=Path(model_path_base+rnd+'_best')),
                SaveModelCallback(learn, every='epoch', monitor='accuracy', name=Path(model_path_base+rnd)),
                CSVLogger(learn, filename=log_path, append=True)]
learn.fit_one_cycle(30, slice(5e-4/50,5e-4), moms=(0.8,0.7), callbacks=callbacks) 
