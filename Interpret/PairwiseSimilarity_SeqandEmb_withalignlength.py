...
def score_alignment(combo):
    ...
    ungapped_alignment_length = len(alignment[ix][0][alignment[ix][3]:alignment[ix][4]].split('-')[0])  
...
def get_scores(combos):
    ...
    return [combo,alignment_score,ungapped_alignment_length,cosine_similarity]

fout = '/scratch/ah1114/LoL/data/PairwiseSimilarities_InterpretSmallSubset_WithMismatchAlignlgth.csv'
...
with open(fout, 'a') as outfile:
    writer = csv.writer(outfile)
    #writer.writerow(['combo','seq_similarity','ungapped_alignment_length','emb_similarity'])
