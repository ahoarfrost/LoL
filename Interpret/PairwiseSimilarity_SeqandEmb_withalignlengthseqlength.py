...
def get_scores(combos):
    combo = list(combos[0])
    alignment_score,ungapped_alignment_length = score_alignment(combos[1])
    cosine_similarity = cosine(combos[2])
    len_seqa = len(combos[1][0])
    len_seqb = len(combos[1][1])
    avg_seqlen = (len_seqa+len_seqb)/2
    return [combo,alignment_score,ungapped_alignment_length,cosine_similarity,len_seqa,len_seqb,avg_seqlen]

fout = '/scratch/ah1114/LoL/data/PairwiseSimilarities_InterpretSmallSubset_WithMismatchAlignlgthSeqlgth.csv'
...
with open(fout, 'a') as outfile:
    writer = csv.writer(outfile)
    #writer.writerow(['combo','seq_similarity','ungapped_alignment_length','emb_similarity','length_seqa','length_seqb','avg_seqlen'])
    writer.writerows(scores)

def get_seq_lengths(df,aligned):
...
lens = get_seq_lengths(df,aligned)
aligned['avg_seqlen'] = lens
aligned['percent_aligned'] = aligned['ungapped_alignment_length']/aligned['avg_seqlen']
