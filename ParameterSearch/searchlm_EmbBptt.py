...
#if stuck with 16GB/32GB, might try doing it in shuffled batches of files?
bs=512 #2048 worked in previous tests, 4096 had memory error; 512 seemed best for speed/accuracy tradeoff for both 5e-3 and 1e-2 lr
max_seqs=None
lrate=5e-3 
num_cycles=20
drop_mult=0.2
...
embeddings = [4,25,50,100,200,300,400]
bptts = [15,30,45,70,100]
...
for emb_sz in embeddings:
    for bptt in bptts:
        print('testing embedding size',emb_sz,'and bptt',bptt)
        model_path = Path('/home/ah1114/LanguageOfLife/ParameterSearch/models/emb'+str(emb_sz)+'_bptt'+str(bptt))
        model_dir = Path('/home/ah1114/LanguageOfLife/ParameterSearch/models/')
        log_path = Path('/home/ah1114/LanguageOfLife/ParameterSearch/train_logs/emb'+str(emb_sz)+'_bptt'+str(bptt))
        lr_plot_out = '/home/ah1114/LanguageOfLife/ParameterSearch/lr_plots/lrplot_emb'+str(emb_sz)+'_bptt'+str(bptt)+'.png'
