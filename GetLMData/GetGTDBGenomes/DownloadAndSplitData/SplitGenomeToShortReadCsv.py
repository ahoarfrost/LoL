...
def chunk_fna(fin, fout, genome_name, distrib, mode="w", shuffle=True):
    ...
    with open(fout, mode) as outfile:
        ...
        writer.writerow(['genome','seq','position'])
        for fna in SeqIO.parse(fin, "fasta"):
            ...
            while start < len(fna.seq):
                ...
                record = [genome_name, subseq, subdescription]
...
files = [x for x in (root/'GTDBrepGenomes').resolve().iterdir()]
...
for ix,fin in enumerate(files):
    ...
    fout = (root/'GTDBrepGenomes_chunked_csv'/str(genome_name+'.csv')).resolve()
